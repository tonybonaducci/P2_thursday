...
def fcv_5(full_set, n_examples, part_lengths,atributes,i,set_type,seed):
      ...
      start = time.time()
      poblacion, mejor_fit, indice_mejor, cromosoma = genetico.AGG_BLX(train_set, test_set, train_set_size, test_set_size,weights,atributes,n_examples)
      end = time.time()   

      print("Poblacion: ",poblacion)
      print("Mejor valor fitness: ",mejor_fit,"Indice: ",indice_mejor,"Cromosoma: ",cromosoma)
      print("Tiempo de ejecucion AGG BLX_alpha: ",end-start)

      start = time.time()
      poblacion, mejor_fit, indice_mejor = genetico.AGG_aritmetico(train_set, test_set, train_set_size, test_set_size,weights,atributes,n_examples)
      end = time.time()   
